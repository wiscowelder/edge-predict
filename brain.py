...
ESPN_STATS_BASE = 'https://site.api.espn.com/apis/v2/sports'
...
def retrain_xgboost_incremental(data: dict) -> dict:
    """
    Retrains XGBoost models for any sport where the training buffer
    has accumulated at least XGB_BUFFER_MIN new resolved games.
    Updates global XGB_MODELS and saves to disk.
    """
    global XGB_MODELS
    if not XGBOOST_AVAILABLE:
        return data

    buffer = data.get('training_buffer') or {}
    updated_sports = []

    for sport in ALL_SPORTS:
        sport_buffer = buffer.get(sport) or []
        if len(sport_buffer) < XGB_BUFFER_MIN:
            continue

        existing_model = XGB_MODELS.get(sport)
        if existing_model is None:
            continue  # Need base model to update

        log.info(f'Incremental XGBoost refit for {sport} ({len(sport_buffer)} new games)...')

        X_rows, y_rows = [], []
        for entry in sport_buffer:
            if not isinstance(entry, dict):
                continue
            fv   = entry.get('features')
            home_won = entry.get('home_won')
            if fv is not None and home_won is not None:
                X_rows.append(np.array(fv, dtype=np.float32))
                y_rows.append(1 if home_won else 0)

        if len(X_rows) >= XGB_BUFFER_MIN:
            X = np.array(X_rows)
            y = np.array(y_rows)
            # Incremental: re-fit with warm_start concept via new XGBClassifier on buffer only
            # (XGBoost doesn't support true warm_start; re-fit on buffer to fine-tune)
            small_model = XGBClassifier(**{**XGB_PARAMS, 'n_estimators': 100})
            small_model.fit(X, y, verbose=False)
            XGB_MODELS[sport] = small_model
            updated_sports.append(sport)
            # Clear buffer for this sport
            data['training_buffer'][sport] = []

    if updated_sports:
        save_xgb_models(XGB_MODELS, XGB_FEATURE_NAMES_DEFAULT)
        log.info(f'Incremental XGBoost update complete: {updated_sports}')
...
def run_foundation_training(data: dict) -> dict:
    ...
    # Store recent game history for advanced model training (transformer.py, graph.py)
    # [-500:] keeps the 500 most recent games per sport to prevent data.json bloating
    log.info('Storing game history for advanced model training...')
    data['all_games_by_sport'] = {
        sport: games[-500:] for sport, games in all_games_by_sport.items() if games
    }
    log.info(f'Stored game history for: {list(data["all_games_by_sport"].keys())}')
...
def run_daily_update(data: dict) -> dict:
    ...
    for sport in ALL_SPORTS:
        ...
        for game in games:
            ...
            # Step 2: Edge — sharp money and sentiment adjustments
            if EDGE_AVAILABLE:
                try:
