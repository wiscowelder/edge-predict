...
try:
    ...
    # simulate.py handles missing nfl_drives / nba_possessions internally:
    # it checks len(drives) < 500 and falls back to rule-based probability
    # tables, then saves a working simulate_models.pkl regardless. The hard
    # disable that was here prevented simulate_models.pkl from ever being
    # written and left cbs.py without score distribution support.
...
def save_data(data: dict) -> bool:
    ...
    # Strip keys that are large in memory and not needed by the frontend.
    # ONLY all_games_by_sport (~12 MB of raw game records) and training_buffer
    # belong here. The advanced module output keys (transformer, graph, stack,
    # explain, simulate_meta, mind) are small metadata dicts that the frontend
    # reads to show model status — they must be saved to data.json.
    HEAVY_KEYS = {'training_buffer', 'all_games_by_sport'}
...
def fetch_espn_past_scores(sport: str, date_str: str) -> List[dict]:
    ...
    Uses sport-specific params (groups=50 for college basketball) to
    ensure ALL D1 games are returned, not just featured matchups.
    """
    path = ESPN_PATHS.get(sport)
    if not path:
        return []
...
def _parse_nflverse_csv(text: str) -> List[dict]:
    ...
    for line in lines[1:]:
        ...
        # ── Historical odds (spread + moneyline) ─────────────────────────────
        # nflverse games.csv already includes spread_line, home_moneyline, and
        # away_moneyline for most games from ~2006 onward. Pre-2006 rows have
        # empty/NA values which safe_float() correctly converts to None, leaving
        # ml_prob and spread_prob as NaN in the XGB feature vector (XGBoost
        # handles NaN natively — no crash, no imputation needed).
        #
        # SIGN CONVENTION:
        #   nflverse spread_line: positive = home favored (e.g. +3.5 = home -3.5)
        #   spread_to_prob() expects: negative = home favored
        #   Therefore: avg_spread = -spread_line  (must negate)
        #
        # _american_to_raw_prob is defined later in this file. Python resolves
        # function names at call time, not definition time, so this is safe.
        raw_spread  = safe_float(col(row, 'spread_line'))
        raw_home_ml = safe_float(col(row, 'home_moneyline'))
        raw_away_ml = safe_float(col(row, 'away_moneyline'))

        hist_avg_spread = None
        if raw_spread is not None:
            # Negate: convert nflverse convention → brain.py spread_to_prob convention
            hist_avg_spread = -raw_spread

        hist_ml_prob = None
        if raw_home_ml is not None and raw_away_ml is not None:
            home_raw = _american_to_raw_prob(raw_home_ml)
            away_raw = _american_to_raw_prob(raw_away_ml)
            if home_raw is not None and away_raw is not None:
                total = home_raw + away_raw
                if total > 0:
                    # Remove vig: each side's implied prob divided by total
                    hist_ml_prob = home_raw / total

        # Build the odds dict only when at least one signal exists.
        # None means this game had no historical odds — build_feature_vector
        # will receive odds_parsed=None and leave ml_prob/spread_prob as NaN.
        historical_odds = None
        if hist_avg_spread is not None or hist_ml_prob is not None:
            historical_odds = {
                'ml_prob':    hist_ml_prob,
                'avg_spread': hist_avg_spread,
            }
...
#   groups=50 → all D1 college basketball (applies to both men's and women's)
# Without 'groups', ESPN returns only a small subset (top-25 teams).
# Note: groups=49 was the original value but returned 0 games. groups=4955 was a
# mistaken TheSportsDB league ID. groups=50 is the correct ESPN D1 umbrella group
# for all college basketball — confirmed as the value used by sportsdataverse's
# wehoop and hoopR packages for full D1 coverage.
    ...
    for sport, games in all_games_by_sport.items():
        ...
        for game in games:
            ...
            fv = build_feature_vector(
                ...
                odds_parsed=game.get('historical_odds'),
...
def run_foundation_training(data: dict) -> dict:
    ...
    # Persist all_games_by_sport to a separate pickle file so that
    # --mode train_models (which runs as a separate GitHub Actions workflow)
    # can access the full game history for transformer.py and graph.py.
    # save_data() intentionally strips all_games_by_sport from data.json
    # (it's ~12 MB and not needed by the frontend), so without this file
    # train_models would receive empty dicts and produce empty {} models.
    # games_cache.pkl is only written here; train_models reads it read-only.
    try:
        with open('games_cache.pkl', 'wb') as _gcf:
            pickle.dump(data['all_games_by_sport'], _gcf, protocol=4)
        _cache_kb = os.path.getsize('games_cache.pkl') / 1024
        log.info(f'games_cache.pkl written ({_cache_kb:.0f} KB) — available for train_models')
    except Exception as _exc:
        log.warning(f'games_cache.pkl write failed (non-fatal): {_exc}')
...
def main() -> None:
    ...
    try:
        if args.mode == 'train':
            ...
            # Restore all_games_by_sport from games_cache.pkl.
            # Foundation training writes this file because save_data() strips
            # all_games_by_sport from data.json (it's ~12 MB and not needed by
            # the frontend). Without restoring it here, transformer.py and
            # graph.py receive empty dicts and produce empty {} models.
            # If the cache file is missing (e.g. first run, or file was deleted),
            # training continues — transformer and graph will fall back to
            # team_history (limited) and log a warning rather than crashing.
            if os.path.exists('games_cache.pkl'):
                try:
                    with open('games_cache.pkl', 'rb') as _gcf:
                        _cached = pickle.load(_gcf)
                    if isinstance(_cached, dict) and _cached:
                        data['all_games_by_sport'] = _cached
                        _sport_counts = {s: len(g) for s, g in _cached.items()}
                        log.info(f'[brain] games_cache.pkl loaded: {_sport_counts}')
                    else:
                        log.warning('[brain] games_cache.pkl was empty — transformer/graph will use fallback')
                except Exception as _exc:
                    log.warning(f'[brain] games_cache.pkl load failed (non-fatal): {_exc}')
            else:
                log.warning('[brain] games_cache.pkl not found — transformer/graph will use team_history fallback')
