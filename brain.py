...
def run_foundation_training(data: dict) -> dict:
    ...
    for sport in ALL_SPORTS:
        ...
        sport_start  = 1999 if sport == 'nfl' else 2002
...
def generate_cbs_picks(data: dict) -> dict:
    ...
    # For each sim: play out all games probabilistically, then score every
    # possible assignment of available_pts to unlocked games.
    # Track cumulative score for each assignment permutation.

    # For n_unlocked <= 16 games, we can't enumerate all n! permutations.
    # Instead: use a greedy simulation approach —
    # Run CBS_SIMS sims, each sim plays all games, then uses the
    # optimal greedy assignment for THAT sim's outcomes.
    # The assignment chosen most often across all sims is the winner.
    # This is equivalent to finding the assignment that maximizes
    # expected total points across all possible week outcomes.

    # assignment_scores[tuple(assignment)] = total_points_earned across all sims
    assignment_scores: Dict[tuple, float] = {}

    for sim_i in range(CBS_SIMS):
        # Simulate outcomes for all unlocked games
        sim_outcomes: Dict[int, str] = {}  # index → winning team
        ...
        # Greedy optimal assignment for this sim's outcomes:
        # Sort unlocked games by "certainty" (1 if sim predicted correctly, 0 if not)
        # weighted by our model probability — assign highest available pts to most certain
        game_scores = []
        for i, gp in enumerate(unlocked):
            sim_winner = sim_outcomes[i]
            our_pick   = gp['pred']['pick']
            will_win   = 1 if sim_winner == our_pick else 0
            # Weight by model confidence so high-confidence correct picks bubble up
            score_key  = gp['pick_prob'] * will_win
            game_scores.append((i, score_key, our_pick))

        # Sort by score_key desc → assign highest available pts
        game_scores.sort(key=lambda x: x[1], reverse=True)
        pts_sorted = sorted(available_pts, reverse=True)

        sim_assignment = tuple(pts_sorted[rank] for rank, (i, _, _) in enumerate(game_scores))
        # Re-order to match unlocked order
        assignment_map = {}
        for rank, (i, _, _) in enumerate(game_scores):
            assignment_map[i] = pts_sorted[rank]
        ...
        key = tuple(assignment_map[i] for i in range(n_unlocked))
