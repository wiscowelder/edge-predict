...
def build_league_graph(data, sport):
    ...
    Primary source: data['all_games_by_sport'] (raw game list from foundation training).
    Fallback: data['team_history'] to derive team set when raw games unavailable.
    Returns (Data, team_to_idx, idx_to_team) or None if insufficient data.
    """
    if not TG_AVAILABLE:
        return None, None, None

    # Build team set from team_history (always populated, used for node features)
    team_history = data.get("team_history", {}).get(sport, {})

    # Get raw games — primary source for edges
    raw_games = data.get("all_games_by_sport", {}).get(sport, [])

    if not raw_games and not team_history:
        return None, None, None

    # Collect all team names from both sources
    all_teams = set(team_history.keys())
    ...
    for game in raw_games:
        ...
        # Derive season from season_year field (NFL) or date string
        date_str = game.get("date", "")
        season_year = game.get("season_year") or (int(date_str[:4]) if len(date_str) >= 4 else current_season_val)
        seasons_ago = current_season_val - int(season_year)
        ...
        raw_margin = abs(float(game.get("margin", 1)))
        norm_margin = np.tanh(raw_margin / 15.0)
        ...
        edge_attr.append([
            ...
            float(home_won),          # home_winner flag
            1500.0 / 2500.0,          # winner_elo_before fallback
            1500.0 / 2500.0,          # loser_elo_before fallback
        ])
...
def _build_training_pairs(data, sport, team_to_idx):
    ...
    Primary source: data['all_games_by_sport'] (raw games from foundation training).
    """
