...
def run_portfolio(data):
    ...
    for sport in ("nfl", "nba", "mlb", "nhl"):
        ...
        for game_id, pred in predictions.items():
            ...
            home_ml   = pred.get("moneyline_home")
            if home_ml is None:
                continue
            ...
            bet_obj = {
                ...
                "confidence_width": float(pred.get("confidence_width", 0.5)),
                "pick_type":        "home_favorite" if win_prob >= 0.5 else "away_favorite",
                "primary_signal":   pred.get("explanation", {}).get("primary_driver", ""),
                "weather_zone":     game.get("weather_zone", ""),
                "game":             game,
            }
    ...
    for bet, frac in zip(candidates, fractions):
        ...
        recommended.append({
            **{k: v for k, v in bet.items() if k != "game"},
