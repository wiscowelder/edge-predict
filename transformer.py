...
def _build_team_histories_from_raw(raw_games, sport):
    ...
    Each entry gets the 12 fields that _normalize_game_features expects.
    """
    score_scale = {"nba": 120.0, "mlb": 10.0, "nhl": 5.0, "ncaabm": 80.0, "ncaabw": 75.0}.get(sport, 100.0)
    team_histories = {}
    # Sort all games chronologically first
    sorted_games = sorted(raw_games, key=lambda g: g.get("date", ""))
    # Running win% and rolling margin trackers per team
    team_records = {}  # team -> [wins, total]
    team_margins = {}  # team -> deque of last 5 margins
    ...
    for game in sorted_games:
        ...
        season     = int(game.get("season_year") or (date[:4] if date else 2020))

        for is_home, team, opp, scored, allowed, won, team_margin in [
            (True,  home, away, home_score, away_score,  home_won,  margin),
            (False, away, home, away_score, home_score, not home_won, -margin),
        ]:
            if team not in team_histories:
                team_histories[team] = []
            if team not in team_records:
                team_records[team] = [0, 0]
            if team not in team_margins:
                team_margins[team] = []

            wins, total = team_records[team]
            cum_win_pct = wins / total if total > 0 else 0.5
            rolling5 = float(np.mean(team_margins[team][-5:])) if team_margins[team] else 0.0
            ...
            # Update trackers
            team_records[team] = [wins + (1 if won else 0), total + 1]
